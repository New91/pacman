...
bool GObject::Achive()
{
	...
	int k = (m_ptCenter.y - (pStage->LD) / 2) % pStage->LD;//计算x坐标的余数
	bool l = (n == 0 && k == 0);
	return l;
}
...
{
	bool b = false;
	// 更新行和列的数据，若是大嘴，则会执行pacman重写的AchiveCtrl函数消除豆子
	AchiveCtrl();
	//判断指令的有效性
...
PacMan::PacMan(int x, int y): GObject(x, y)
{
